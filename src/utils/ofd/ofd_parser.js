  ...
  for (const stamp of stampAnnot) {
    if (stamp.sealObj && Object.keys(stamp.sealObj).length > 0) {
      if (stamp.sealObj.type === "ofd") {
        ...
        stamp.sealObj.type === "png" ||
        stamp.sealObj.type === "gif" ||
        stamp.sealObj.type === "jpg" ||
        stamp.sealObj.type === "svg" ||
        stamp.sealObj.type === "bmp"
      ) {
        //兼容gif、jpg、svg、bmp
        // let img = 'data:image/png;base64,' + btoa(String.fromCharCode.apply(null, stamp.sealObj.ofdArray));
        let img =
          "data:image/png;base64," + Uint8ToBase64(stamp.sealObj.ofdArray);
        let stampArray = [];
...
export const getDocumentRes = async function ([
  zip,
  doc,
  Document,
  stampAnnot,
...
export const getPublicRes = async function ([
  zip,
  doc,
  Document,
  stampAnnot,
  ...
  if (publicResPath) {
    ...
    if (zip.files[publicResPath]) {
      ...
      compositeGraphicUnits = compositeGraphicUnits.concat(
        getCompositeObject(publicResObj)
      );
    }
  }
  return [
...
export const getTemplatePage = async function ([
  zip,
  doc,
  Document,
  stampAnnot,
    ...
    for (const item of array) {
      if (item) {
        drawParamResObj[item["@_ID"]] = {
          ...
          FillColor: item["ofd:FillColor"]
            ? item["ofd:FillColor"]["@_Value"]
            : "",
          StrokeColor: item["ofd:StrokeColor"]
            ? item["ofd:StrokeColor"]["@_Value"]
            : "",
    ...
    for (const item of array) {
      if (item) {
        ...
        if (item["@_Type"].toLowerCase() === "image") {
          ...
          if (
            (format &&
              (format.toLowerCase() === "gbig2" ||
                format.toLowerCase() === "jb2")) ||
            (ext &&
              (ext.toLowerCase() === "jb2" || ext.toLowerCase() === "gbig2"))
          ) {
    ...
    if (zip.files[signatures]) {
      ...
      for (const sign of signatureArray) {
        if (sign) {
          ...
          stampAnnot.push(
            await getSignatureData(zip, signatureLoc, signatureID)
          );
        }
      }
    }
...
const getSignatureData = async function (zip, signature, signatureID) {
  ...
  if (!zip.files[signedValue]) {
    signedValue = `${signature.substring(
      0,
  ...
  data["json"]["ofd:Signature"]["ofd:SignedInfo"]["ofd:References"][
    ...
    if (
      ...
      Object.keys(reference["@_FileRef"]).length == 0
    ) {
      return true;
    }
  ...
  return {
    ...
    signedInfo: {
      ...
      SignatureMethod:
        data["json"]["ofd:Signature"]["ofd:SignedInfo"]["ofd:SignatureMethod"],
      SignatureDateTime:
        data["json"]["ofd:Signature"]["ofd:SignedInfo"][
...
const getSealDocumentObj = function (stampAnnot) {
  return new Promise((resolve, reject) => {
    pipeline
      .call(
        this,
        async () => await unzipOfd(stampAnnot.sealObj.ofdArray),
        getDocRoots,
        parseSingleDoc
      )
      .then((res) => {
...
const getJsonFromXmlContent = async function (zip, xmlName) {
  return new Promise((resolve, reject) => {
    zip.files[xmlName].async("string").then(
      function (content) {
        let ops = {
...
const parseJbig2ImageFromZip = async function (zip, name) {
  return new Promise((resolve, reject) => {
    zip.files[name].async("uint8array").then(
      function (bytes) {

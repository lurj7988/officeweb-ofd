...
export const digestByteArray = function (data, hashedBase64, checkMethod) {
  ...
  if (
    checkMethod.indexOf("1.2.156.10197.1.401") >= 0 ||
    checkMethod.indexOf("sm3") >= 0
  ) {
...
export const SES_Signature_Verify = function (SES_Signature) {
  try {
    ...
    if (
      signAlg.indexOf("1.2.156.10197.1.501") >= 0 ||
      signAlg.indexOf("sm2") >= 0
    ) {
      let sigValueHex = SES_Signature.signature
        .replace(/ /g, "")
        .replace(/\n/g, "");
      ...
      const cert =
        SES_Signature.realVersion < 4
          ...
          : SES_Signature.cert;
      let publicKey = cert.subjectPublicKeyInfo.subjectPublicKey
        .replace(/ /g, "")
        .replace(/\n/g, "");
